...
export function seedCards(): void {
  const insert = db.prepare(
    "INSERT OR IGNORE INTO cards_master (id, name, number, set_name, set_code, rarity) VALUES (?, ?, ?, ?, ?, ?)"
  );
  const insertMany = db.transaction(() => {
    let inserted = 0;
    for (const card of cards) {
      const result = insert.run(uuidv4(), card.name, card.number, card.set_name, card.set_code, card.rarity);
      if (result.changes > 0) inserted++;
    }
    return inserted;
  });
  const inserted = insertMany();
  console.log(`Seeded ${inserted} cards`);
}
